...
class CertificatePinningService {
        ...
        try {
          ...
          if (pins.isNotEmpty) {
            SecureLogger.logSecurity(
              'Applying certificate pinning for $host',
              data: {'pins': pins, 'url': options.uri.toString()},
            );
  ...
  static Future<void> _applyCertificatePinning(
    ...
    try {
      ...
      if (options.uri.scheme == 'http') {
        SecureLogger.logSecurity(
          'Skipping certificate pinning for HTTP request',
        );
        return;
      }
  ...
  static Future<void> _pinCertificate(String url, List<String> pins) async {
    try {
      ...
      if (!isPinned) {
        SecureLogger.logSecurity(
          'Certificate fingerprint mismatch for $url',
          data: {'expected': pins, 'actual': fingerprintString},
        );
        throw CertificatePinningException(
          'Certificate pinning verification failed for $url',
        );
      }

      SecureLogger.logSecurity(
        'Certificate pinning verification successful for $url',
      );
      await socket.close();
    } catch (e) {
  ...
  static String _getCertificateFingerprint(X509Certificate certificate) {
    try {
      ...
      throw CertificatePinningException(
        'Failed to calculate certificate fingerprint: $e',
      );
    }
  }
  ...
  static Future<String?> getCertificateFingerprint(
    String host, {
    int port = 443,
  }) async {
    try {
      ...
      SecureLogger.logInfo(
        'Certificate fingerprint for $host:$port: $fingerprintString',
      );
      ...
      SecureLogger.logError(
        'Failed to get certificate fingerprint for $host:$port',
        error: e,
      );
      return null;
    }
  ...
  static Future<bool> testCertificatePinning(
    String host, {
    int port = 443,
  }) async {
    try {
      ...
      SecureLogger.logError(
        'Certificate pinning test failed for $host',
        error: e,
      );
      return false;
    }
