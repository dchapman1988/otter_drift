...
class RetryService {
  ...
  static Future<T> executeWithRetry<T>(
    ...
    for (int attempt = 1; attempt <= retryConfig.maxAttempts; attempt++) {
      try {
        SecureLogger.logDebug(
          'Attempting $name (attempt $attempt/${retryConfig.maxAttempts})',
        );
    ...
    SecureLogger.logError(
      '$name failed after ${retryConfig.maxAttempts} attempts',
    );
    throw lastException ??
  ...
  static Duration _calculateDelay(int attempt, RetryConfig config) {
    ...
    return Duration(
      milliseconds: finalDelay.round().clamp(0, config.maxDelay.inMilliseconds),
    );
  }
